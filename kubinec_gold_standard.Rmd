...
I show that entropy when applied to causal graphs returns intuitive results that provide a foundation for how scholars actuall engage in research. In a given caual graph, it is entirely possible that an observational analysis, which can only show associations, will reduce the entropy of a causal graph as much as, or more than, an experiment that is truly causally identified. As the aim of the social sciences should be to reduce our uncertainty in understanding how the social world operates, we would benefit from applying the word causal more liberally rather than to force research designs into a hierarchical mold of descriptive vs. causal inference.
...
> The potential outcomes framework, and experments in particular, have many good properties.

> Ergo, the potential outcomes framework is causal inference.

The subtlety is that causal inference and the potential outcomes framework, or other causal paradigms, are never formally defined as equivalent. Rather, the potential outcome framework and RCTs are praised for several noteworthy properties, and it is easy to move from praise of those properties to the belief that a method with such noteworthy properties must therefore be causal. But while causal inference is an excellent aim for the social sciences, and one we should strive for, that does not mean that all excellent endeavors are by that definition causal. In other words, the conflations of potential outcomes and causal inference comes from the fallacy of affirming the consequent.
