...
The source of this difficulty, I argue, is that the slippery nature of causality leads social scientists to make a subtle but significant logical error. Underlying much of the recent writing on causal inference is the following syllogism:

> Causal inference is a good aim for social science.

> Randomized experiments have many good properties.

> Ergo, randomized experiments are causal inference.

The subtlety is that causal inference and randomized experiments, or other causal paradigms, are never formally defined as equivalent. Rather, RCTs are praised for several noteworthy properties, and it is easy to move from praise of those properties to the belief that a method with such noteworthy properties must therefore be causal. But while causal inference is an excellent aim for the social sciences, and one we should strive for, that does not mean that all excellent endeavors are by that definition causal. In other words, the conflations of RCTs and causal inference comes from the fallacy of affirming the consequent.
...
makeDAG(dagname='example2.png',filetype='png',
        text.nodes=unctrl,
        box.nodes=ctrl,
        solid.edges = meas,
        embed=T)
...
Rather than cast aspersions on RCTS and other forms of counterfactual inference, it is best to think of these methods as providing a very helpful and important component of what is meant by causality. RCTs are also the only means available for addressing unmeasurable selection attributes, such as socially undesirable preferences people may be unwilling to report truthfully. Under ideal conditions, such as with large samples, low attrition rates, and cleanly measured treatments, RCTs provide very strong knowledge about *certain types* of causal relations, but they cannot give us the final word. For that reason, I turn next to other conceptual paradigms for understanding causal relations.
...
In addition, like the other two silver standards, under ideal conditions it can provide strong evidence of causality. In the social sciences this entails collecting exhaustive evidence on a person's decision-making in what has come to be called process-tracing. Process-tracing methodologists refer to the idea of a "smoking gun" as the kind of evidence that establishes causality within this framework, such as obtaining a private diary of an important leader that describes in detail why they made their decisions [@vanevera1997; @collier2010; @Bennett2014; @humphreys2015]. These ideal conditions are relatively rare, but like experimental and observational approaches, we can have confidence in inferring *a kind* of causality when we have all the information we might want about how $X$ changed into $Y$.
...
However, suppose that if we conducted an observational data analysis, we could increase or decrease the probability of the links between leader selection and economic benefits (E), leadership quality (Q), ideology (I), ethnic affinity (A) and conflict (C) from $\frac{1}{36}$ to $\frac{1.5}{36}$ while lowering the opposite links to $\frac{0.5}{36}$. That would result in the following change in entropy:
