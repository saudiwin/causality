...
In this paper, I propose a new method for understanding preregistration as involving a registration of prior researcher knowledge that is expressed in the form of a causal graph. This method builds on recent advances in social science research design that is increasing the capability of tools for applied researchers to diagnose issues and construct robust theories [@blair2023; @humphreys2023; @scheel2022; @vowels2023]. With a preregistered causal graph along with suitable priors over the relationships between variables [@humphreys2023; @vowels2023], authors can then employ metrics based on entropy to determine the level of causal knowledge that a particular study would contribute [@buzbas2024]. Importantly, defining causal knowledge as a function of an entire causal graph—as opposed to a single experimental treatment—allows for a range of analyses to be incorporated into potential learning without requiring explicit preregistration. In fact, I show that this method applies equally well to observational, experimental, and qualitative research designs.
...
The solution proposed in this paper is to preregister causal graphs for all kinds of research, whether observational, experimental, or even qualitative (mechanistic). A causal graph does not need to be absolutely true in an epistemic sense to still be a useful guide for theory-informed research [@vowels2023]. While this method does require the researcher to formalize their knowledge of the data-generating process, the preregistration of causal graphs provides a path forward for permitting exploratory analyses that avoid post-hoc theoretical justifications for their decisions [@velicuMakingMisconductMake2025]. Furthermore, as I show in this article, preregistering causal graphs permits more holistic statements of a study's contribution to causal learning; i.e., incorporating studies that are both experimental *and* non-experimental.
...
Though not the only possible choice, entropy is an excellent statistical summary measure that can be used to compare causal graphs from before and after studies are conducted. Entropy is a concept with a long history in statistics, though in this paper it is used primarily as a way of making nuanced statements about the sum total of uncertainty in a causal system. Statistical, or Shannon, entropy provides a measure of the "information" in a random variable in terms of the deviation of a set of observations from pure random noise [@shannon1948].
...
The formula in (\ref{shannon}) is unfortunately not intuitive, in part because the units of entropy are in logarithms. Because I am interested in entropy as a framework, I use the logarithmic base of 1.01:
...
Once the analyst has chosen the set of variables which pertain to the research question, it is possible to calculate entropy of the preregistered causal graph and a causal graph that is estimated after the new data are collected. I now formally define the causal entropy measure as the Shannon entropy $H(\cdot)$ of an ordered set of variables (nodes) $\{x_1, ... x_N\}$ that can be represented by a causal graph $v$ which meets all of Pearl's requirements: it includes a set of directed edges $e \in E$ between each variables $x_i$ in the graph and is acyclic. Because the graph is ordered, there is necessarily at least one terminal node that represents the outcome that the graph seeks to explain. We can then take the Shannon entropy $H(\cdot)$ of the joint distribution of these variables (indexed by $i \in N$) with respect to a given terminal node or outcome, which can be denoted $P(v)$, following Pearl's notation:
...
To make this method concrete, I next provide a case study of how causal graphs and entropy calculations can be applied to actual research problems. In addition to the case study described in this section, I include a case study in SI Section 2 with a more complex causal graph. While these case studies do not have preregistered causal graphs as this method of preregistration is new, they do reveal how to construct a preregistered causal graph for a given research design and how to incorporate experimental, observational, and even qualitative analyses into the calculation of causal learning via entropy.
